...
public class EnemySilverfish extends EntitySilverfish implements IBasicEnemy {
	...
	public EnemySilverfish(org.bukkit.World w, int wave) {
        ...
        if(wave < 30) {
        	...
        	stage = 4;
        	this.getAttributeInstance(GenericAttributes.maxHealth).setValue(6*(wave/2));
	...
	public HashMap<ItemStack, Integer> getDrops() {
		ItemStack dia = new ItemStack(Material.STONE);
		ItemStackUtil.addNameToItem(dia, ChatColor.GOLD + "Old Rock");
		ItemStackUtil.addLoreToItem(dia, ChatColor.AQUA + "Rare Item");
		ItemStackUtil.addLoreToItem(dia, ChatColor.YELLOW + "Perhaps it can be cracked open somehow?");
		drops.put(dia, 1);
		return drops;
	}
